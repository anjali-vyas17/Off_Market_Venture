...
class UnlistedDealLedger(Document):
    def validate(self):
        self._auto_fill_bank_accounts()
        self.calculate_totals()
        self.validate_roles()

    def _auto_fill_bank_accounts(self):
        """Fetch bank account details from linked counterparty profiles (only if empty)"""
        if self.seller_profile and not self.seller_bank_account:
            try:
                cp = frappe.get_doc("Counterparty Profile", self.seller_profile)
                if cp.bank_accounts:
                    b = cp.bank_accounts[0]
                    self.seller_bank_account = f"{b.bank_name} - Acc: {b.account_number} (IFSC: {b.ifsc_code})"
            except Exception:
                pass

        if self.buyer_profile and not self.buyer_bank_account:
            try:
                cp = frappe.get_doc("Counterparty Profile", self.buyer_profile)
                if cp.bank_accounts:
                    b = cp.bank_accounts[0]
                    self.buyer_bank_account = f"{b.bank_name} - Acc: {b.account_number} (IFSC: {b.ifsc_code})"
            except Exception:
                pass
    ...
    def calculate_totals(self):
        ...
        referral = float(self.referral_fee or 0)
        gross_spread = self.buyer_gross_value - self.seller_gross_cost
        self.net_arbitrage = round(gross_spread - brokerage - expenses - referral, 2)
